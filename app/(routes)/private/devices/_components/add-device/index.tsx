...
export function AddDevice({ browsers, refreshDevices }: { browsers: IBrowser[], refreshDevices: () => void; }) {
    ...
    async function onSubmit(values: z.infer<typeof deviceSchema>) {
        setIsLoading(true)
        try {
            const response = await addDeviceService({ ...values, browsers: selectedBrowsers });
            if (response) {
                refreshDevices();
            ...
            <SheetContent className="w-full !max-w-full md:w-[550px] md:!max-w-[550px]">
                <SheetHeader>
                    ...
                    <SheetDescription className="text-left">
                        Expand your device inventory! The more devices you add, the greater your chances of receiving project recommendations.
                    </SheetDescription>
                </SheetHeader>
                <div className="mt-4">
                    <Form {...form}>
                        <form onSubmit={form.handleSubmit(onSubmit)} method="post">
                            ...
                            <div className="my-2">
                                <Label className={isInvalidBrowsers ? 'text-destructive' : ""}>Browsers</Label>
                                <MultiSelect
                                    ...
                                    className="mt-2"
                                />
                                {isInvalidBrowsers ? <FormMessage className="mt-2">Please select at least one browser</FormMessage> : null}
                            </div>
                            ...
                            <div className="mt-8 w-full flex justify-end gap-2">
                                <SheetClose asChild>
                                    <Button disabled={isLoading} type="button" variant={'outline'} size="lg" className="w-full md:w-fit">Cancel</Button>
                                </SheetClose>
                                <Button disabled={isLoading} type="submit" size="lg" onClick={() => validateBrowsers()} className="w-full md:w-fit">
