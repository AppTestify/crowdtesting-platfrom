...
import { Loader2, Monitor, Smartphone, Globe, Wifi, Settings, Edit } from "lucide-react";
...
export function EditDevice({
  ...
  const { name, os, version, country, city, network } = device;
  ...
  async function onSubmit(values: z.infer<typeof deviceSchema>) {
    setIsLoading(true);
    try {
      const response = await updateDeviceService(deviceId, {
        ...values,
  ...
  const resetForm = () => {
    ...
    setSelectedBrowsers(device.browsers);
  };
          ...
          <div className="relative flex items-start gap-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              <DialogDescription className="text-gray-600 text-sm">
                Update device settings and browser compatibility for comprehensive testing coverage
              </DialogDescription>
            </div>
          </div>
        </div>
