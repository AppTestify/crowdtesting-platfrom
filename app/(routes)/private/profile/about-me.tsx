    ...
    <div className="p-8 pt-0">
      <div className=" w-[1018px] flex justify-between items-center mb-2 ">
        <label className="text-sm">
          {yourSelfData.length === 0 && "No Info Found ! , Please Update about yourself"}
        </label>
        <Sheet>
          <SheetTrigger asChild>
            <Button className="h-6 text-xs">{yourSelfData.length === 0 ? "Update" : "Edit"}</Button>
          </SheetTrigger>
          <SheetContent>
            <SheetHeader>
              <SheetTitle>{isEditMode ? "Edit" : "Add"} About Yourself</SheetTitle>
            </SheetHeader>
            <div className="grid gap-4 py-4">
              <div className="flex-1 items-center">
                <Textarea value={yourseSelf} onChange={(e) => setYourself(e.target.value)} className="w-full h-80" placeholder="Type your self here." />
              </div>
            </div>
            <SheetFooter>
              <SheetClose asChild>
