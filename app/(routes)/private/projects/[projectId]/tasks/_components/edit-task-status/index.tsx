...
import { updateTaskStatusService } from '@/app/_services/task.service';
...
export default function EditTaskStatus({
    ...
    const onSubmit = async (values: z.infer<typeof taskStatusSchema>) => {
        setIsLoading(true);
        try {
            const response = await updateTaskStatusService(projectId, task?.id, values);
            if (response) {
                refreshTasks();
            ...
            <Dialog open={sheetOpen} onOpenChange={setSheetOpen}>
                <DialogContent className="sm:max-w-[425px]">
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            Update the status of the task to keep track of the progress.
                        </DialogDescription>
                    </DialogHeader>
                    <Form {...form}>
