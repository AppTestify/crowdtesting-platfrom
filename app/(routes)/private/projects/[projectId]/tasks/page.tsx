...
export default function Tasks() {
  ...
  const columns: ColumnDef<ITask>[] = [
    ...
    },
    {
      ...
      cell: ({ row }: { row: any }) => {
        ...
        return (
          <div title={issueTitle} className="capitalize">
            {issueTitle?.length > 30
              ? `${issueTitle?.substring(0, 30)}...`
              : issueTitle}
          </div>
        );
      },
    },
    {
      ...
      cell: ({ row }: { row: any }) => (
        <div className="capitalize">
          <ExpandableTable
            row={row?.original?.requirementIds as IRequirement[]}
          />
        </div>
      ),
    },
  ...
  const [isEditOpen, setIsEditOpen] = useState<boolean>(false);
    const [editTask, setEditTask] = useState<ITask | null>(
      null
    );
