...
import { deleteReportAttachmentService } from "@/app/_services/report-attachment.service";

export function ReportAttachmentRowActions({
    row,
    refreshReports,
    reportId,
    isView,
}: {
    row: Row<IReportAttachmentDisplay>;
    refreshReports: () => void;
    reportId: string;
    isView: boolean;
}) {
    ...
    const deleteAttachment = async () => {
        try {
            setIsLoading(true);
            const response = await deleteReportAttachmentService(projectId, reportId, fileId);
