  ...
  const columns: ColumnDef<ITestCyclePayload>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return (
          <div title={description} className="capitalize line-clamp-2">
    ...
    },
    {
      ...
      header: "Type",
      cell: ({ row }) => (
        <div title={row.original?.type} className="">
          {row.original?.type}
        </div>
      ),
    },
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div className="">
          {row.original?.testCycle?.testCases &&
          row.original?.testCycle?.testCases.length > 0 ? (
            <Link
              href={`/private/projects/${projectId}/test-executions/${row.original?.id}`}
            >
