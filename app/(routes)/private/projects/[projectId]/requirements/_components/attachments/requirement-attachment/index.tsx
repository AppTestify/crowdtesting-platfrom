...
import {
  IRequirementAttachmentDisplay,
  RequirementAttachmentsProps,
} from "@/app/_interface/requirement";
...
export default function RequirementAttachments({
  requirementId,
  isUpdate,
  isView,
  setAttachmentsData,
  ...
  const columns: ColumnDef<IRequirementAttachmentDisplay>[] = [
    ...
    },
    {
      ...
      enableHiding: false,
      cell: ({ row }) => (
        <RequirementAttachmentRowActions
          row={row}
  ...
  const getAttachments = async () => {
    try {
      ...
      const response = await getRequirementAttachmentsService(
        projectId,
        requirementId
      );
    ...
    <div className="mt-4">
      ...
      <div className="mt-2">
        {table.getRowModel().rows?.length > 0 ? (
          ...
          <div className="text-center h-20">Loading</div>
        ) : null}
      </div>
