...
export function TestCaseData({ testCaseId }: { testCaseId: string }) {
    ...
    async function onSubmit(values: z.infer<typeof testPlanSchema>) {
        ...
        try {
            ...
            const response = await addTestCaseDataService(projectId, testCaseId, payload);
            if (response) {
                resetForm();
                getTestCaseData();
        ...
        <div>
            <div>
                {testCaseData.length > 0 &&
                    <TestCaseDataTable testCaseData={testCaseData} refreshTestCaseData={refreshTestCaseData} />
                }
            </div>
            <div className="mt-4">
                <Form {...form}>
                                    ...
                                    <div className="flex  mt-4">
                                        {index === fields.length - 1 && (
                                            <Button
                                                ...
                                                size="sm"
                                                className="mr-4"
                                                onClick={() => append({ name: "", type: "", inputValue: "", validation: [""], description: "" })}
