...
export async function GET(
  ...
  try {
    ...
    // Another data
    const testCaseIds = response
      .filter((res) => res?.testCaseId)
      .map((res) => res.testCaseId._id);
    ...
    const allData = await Promise.all(
      response.map(async (res) => {
        ...
        const stepsForTestCase = testCaseStep.filter(
          (step) =>
            res.testCaseId &&
            step.testCaseId.toString() === res.testCaseId._id.toString()
        );

        const dataForTestCase = testCaseData.filter(
          (data) =>
            res.testCaseId &&
            data.testCaseId.toString() === res.testCaseId._id.toString()
        );
