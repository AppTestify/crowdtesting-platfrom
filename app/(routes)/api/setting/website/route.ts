...
export async function GET(req: Request) {
  try {
    ...
    const response = await Website.findOne({}).lean();
    ...
    if (response?.logo?.cloudId) {
      fileResponse = await attachmentService.fetchFileAsBase64(
        response?.logo?.cloudId
      );
    }

    return Response.json({
      ...response,
      logo: { ...response?.logo, data: fileResponse },
    });
  } catch (error: any) {
    return errorHandler(error);
  }
