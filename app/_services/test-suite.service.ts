...
export const getTestSuiteService = async (
  projectId: string,
  index: Number,
  pageSize: Number,
  globalFilter: string
  ...
  try {
    const response = await genericGet(
      `${TEST_SUITE_ENPOINT(projectId)}${PAGINATION_QUERY_ENDPOINT(
        index,
        pageSize
...
export const addTestSuiteService = async (
  projectId: string,
  body: ITestSuitePayload
): Promise<any> => {
  try {
    const response = await genericPost(
      `${TEST_SUITE_ENPOINT(projectId)}`,
      body
    );
    return response || {};
  } catch (error) {
...
export const deleteTestSuiteService = async (
  ...
  try {
    const response = await genericDelete(
      `${TEST_SUITE_ENPOINT(projectId)}/${testSuiteId}`
    );
    return response || {};
  } catch (error) {
...
export const getTestWithoutPaginationSuiteService = async (
  projectId: string
): Promise<any> => {
  try {
    const response = await genericGet(
      `${TEST_SUITE_ENPOINT(projectId)}/without-pagination`
    );
    return response || [];
  } catch (error) {
