...
import {
  ITestCaseStepPayload,
  ITestCaseStepSequencePayload,
} from "../_interface/test-case-step";
import {
  genericDelete,
  genericGet,
  genericPost,
...
export const addTestCaseStepService = async (
  projectId: string,
  testCaseId: string,
  ...
  try {
    const response = await genericPost(
      `${TEST_CASE_STEP_ENPOINT(projectId, testCaseId)}`,
      body
    );
    return response || {};
  } catch (error) {
...
export const getTestCaseStepService = async (
  projectId: string,
  testCaseId: string
): Promise<any> => {
  try {
...
export const updateTestCaseSequenceService = async (
  ...
  try {
    const response = await genericPut(
      `${TEST_CASE_STEP_ENPOINT(projectId, testCaseId)}/reorder`,
      body
    );
    return response || {};
  } catch (error) {
