...
import { ProjectNav } from "@/app/_components/app-sidebar/project-nav";
import {
  Sidebar,
  SidebarContent,
  SidebarFooter,
...
export function AppSidebar({ ...props }: React.ComponentProps<typeof Sidebar>) {
  ...
  const isInsideProject = pathname.includes('/private/projects/') && pathname.split('/').length > 3;
    ...
    <>
      <Sidebar collapsible="icon" {...props} className="border-r-0">
        ...
        <SidebarContent className="px-2 py-2">
          {isInsideProject ? (
            <ProjectNav onNavClick={handleNavClick} />
          ) : (
